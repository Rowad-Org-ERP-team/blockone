...
class SeminarFees(Document):
	...
	def set_missing_accounts_and_fields(self):
		...
		if not self.attendant_email:
			self.attendant_email = self.get_attendant_emails()

	def get_attendant_emails(self):
		attendant_emails = frappe.db.sql_list("""
			select g.email_address
			from `tabGuardian` g, `tabAttendant Guardian` sg
			where g.name = sg.guardian and sg.parent = %s and sg.parenttype = 'Attendant'
			and ifnull(g.email_address, '')!=''
		""", self.attendant)

		attendant_email_id = frappe.db.get_value("Attendant", self.attendant, "attendant_email_id")
		if attendant_email_id:
			attendant_emails.append(attendant_email_id)
		if attendant_emails:
			return ", ".join(list(set(attendant_emails)))
		else:
			return None
	...
	def on_submit(self):
		...
		if self.send_payment_request and self.attendant_email:
			pr = make_payment_request(party_type="Attendant", party=self.attendant, dt="SeminarFees",
					dn=self.name, recipient_id=self.attendant_email,
	...
	def make_gl_entries(self):
		if not self.grand_total:
			return
		attendant_gl_entries =  self.get_gl_dict({
			"account": self.receivable_account,
			"party_type": "Attendant",
			"party": self.attendant,
		...
		make_gl_entries([attendant_gl_entries, fee_gl_entry], cancel=(self.docstatus == 2),
			update_outstanding="Yes", merge_entries=False)


def get_fee_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by="modified"):
	user = frappe.session.user
	student = frappe.db.sql("select name from `tabAttendant` where attendant_email_id= %s", user)
	if student:
		return frappe. db.sql('''
			select name, program, due_date, grand_total - outstanding_amount as paid_amount,
			outstanding_amount, grand_total, currency
			from `tabFees`
			where attendant= %s and docstatus=1
			order by due_date asc limit {0} , {1}'''
			.format(limit_start, limit_page_length), attendant, as_dict = True)
